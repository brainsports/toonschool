...
import { orgAdminNotificationService, type OrgAdminNotification } from '../services/orgAdminNotificationService'
import { useAuth } from '../../../shared/contexts/AuthContext'

interface OrgAdminNotificationInboxProps {
  onClose: () => void
  onCountChange?: (count: number) => void
}

export default function OrgAdminNotificationInbox({ onClose, onCountChange }: OrgAdminNotificationInboxProps) {
  const { profile } = useAuth()
  const [notifications, setNotifications] = useState<OrgAdminNotification[]>([])
  const [loading, setLoading] = useState(true)
  const [selectedNoti, setSelectedNoti] = useState<OrgAdminNotification | null>(null)

  useEffect(() => {
    if (profile?.id && profile?.role === 'org_admin') {
      loadNotifications()
    }
  }, [profile])
  ...
  const loadNotifications = async () => {
    try {
      setLoading(true)
      const data = await orgAdminNotificationService.getNotifications(profile!.id, profile!.organization_id || null)
  ...
  const handleRead = async (noti: OrgAdminNotification) => {
    if (!noti.is_read) {
      try {
        await orgAdminNotificationService.markAsRead(profile!.id, noti.id)
  ...
  const handleDelete = async (noti: OrgAdminNotification, e?: React.MouseEvent) => {
    if (e) e.stopPropagation()
    
    if (window.confirm('이 알림을 내 알림함에서 삭제하시겠습니까?')) {
      try {
        await orgAdminNotificationService.hideNotification(profile!.id, noti.id)
