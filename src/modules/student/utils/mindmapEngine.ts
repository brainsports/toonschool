...
import { BRANCH_COLOR_KEYS, MINDMAP_LIMITS, LAYOUT, MAX_CHILD_BRANCHES } from '../data/mindmapConfig';
...
export function addNode(nodes: MindmapNode[], opts: AddNodeOptions): { nodes: MindmapNode[]; node: MindmapNode | null; reason?: string } {
  ...
  // 한 가지에는 작은 가지를 최대 10개까지만 만들 수 있음
  if (opts.type !== 'main' && opts.type !== 'central' && opts.type !== 'thought') {
    const directChildren = nodes.filter(n => n.parentId === parent.id);
    if (directChildren.length >= MAX_CHILD_BRANCHES) {
      return { nodes, node: null, reason: '한 가지에는 작은 가지를 10개까지 만들 수 있어요.' };
    }
  }
