...
import { Search, X, Pin, Bookmark, BookMarked } from 'lucide-react'
...
export default function ToonVocabularyPanel(props: ToonVocabularyPanelProps) {
  const {
    ...
    onPickRecent, onClearRecents, onClose, onSave, onLearnMore, onOpenWordbook,
    saveState, saveMessage, selSense, onSelSenseChange, panelId, inputRef, variant = 'desktop',
  } = props
  ...
  return (
    <section id={panelId} className={`tv-panel tv-panel--${variant}`} role="dialog" aria-modal="true" aria-labelledby={`${panelId}-title`}>
      {variant === 'mobile' && <div className="tv-sheet-handle" aria-hidden="true" />}
      {/* 헤더 */}
      <div className="tv-header">
        <img className="tv-header__mascot" src={MASCOT} alt="" />
        <div className="tv-header__titles">
          <div className="tv-header__title-row">
            <span id={`${panelId}-title`} className="tv-header__title">툰어휘사전</span>
            <Pin className="tv-header__pin" size={18} />
          </div>
          <div className="tv-header__subtitle">모르는 단어를 쉽게 알려줄게!</div>
        </div>
        <button className="tv-header__close" onClick={onClose} aria-label="툰어휘사전 닫기">
          <X size={18} />
        </button>
      </div>
      ...
      <div className="tv-footer">
        ...
        >
          <Bookmark size={18} /> {saveLabel === '저장하기' ? '나의 단어장에 저장' : saveLabel}
        </button>
        <button className="tv-btn tv-btn--primary" onClick={onLearnMore} disabled={!hasResults}>
          <Search size={18} /> 더 알아보기
        </button>
      </div>
      <button className="tv-wordbook-link" type="button" onClick={onOpenWordbook}>
        <BookMarked size={17} /> 나의 단어장 보기
      </button>
    </section>
  )
}
