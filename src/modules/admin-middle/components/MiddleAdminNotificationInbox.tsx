...
import { middleAdminNotificationService, type MiddleAdminNotification } from '../services/middleAdminNotificationService'
import { useAuth } from '../../../shared/contexts/AuthContext'

interface MiddleAdminNotificationInboxProps {
  onClose: () => void
  onCountChange?: (count: number) => void
}

export default function MiddleAdminNotificationInbox({ onClose, onCountChange }: MiddleAdminNotificationInboxProps) {
  const { profile } = useAuth()
  const [notifications, setNotifications] = useState<MiddleAdminNotification[]>([])
  const [loading, setLoading] = useState(true)
  const [selectedNoti, setSelectedNoti] = useState<MiddleAdminNotification | null>(null)

  useEffect(() => {
    if (profile?.id && profile?.role === 'middle_admin') {
      loadNotifications()
    }
  }, [profile])
  ...
  const loadNotifications = async () => {
    try {
      setLoading(true)
      const data = await middleAdminNotificationService.getNotifications(profile!.id)
  ...
  const handleRead = async (noti: MiddleAdminNotification) => {
    if (!noti.is_read) {
      try {
        await middleAdminNotificationService.markAsRead(profile!.id, noti.id)
  ...
  const handleDelete = async (noti: MiddleAdminNotification, e?: React.MouseEvent) => {
    if (e) e.stopPropagation()
    
    if (window.confirm('이 알림을 내 알림함에서 삭제하시겠습니까?')) {
      try {
        await middleAdminNotificationService.hideNotification(profile!.id, noti.id)
          ...
          {loading ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {notifications.map(noti => (
                ...
                >
                  {!noti.is_read && (
                    <div style={{ width: 8, height: 8, background: '#7c3aed', borderRadius: '50%', position: 'absolute', top: 16, right: 16 }}></div>
                  )}
