...
export default function ToonEditor() {
  ...
  const [title, setTitle] = useState('?섏쓽 泥??곗＜ 紐⑦뿕 留뚰솕')
  const [cuts, setCuts] = useState<ToonCut[]>([
    { id: 'cut-1', description: '?곗＜??議곗쥌???? 源쒖쭩 ?? ?쇨뎬???섍퀬 ?덈뒗 二쇱씤怨?罹먮┃??, dialogue: '?곗＜???곕즺媛 ?쇰쭏 ???⑥븯?뽰븘?! ?댁찈吏?' },
    { id: 'cut-2', description: '李쎈컰?쇰줈 ?멸퀎 ?됱꽦???⑸웾??遺됱? ???吏媛 ?쇱퀜吏??紐⑥뒿', dialogue: '?湲?遺됱? ?됱꽦??鍮꾩긽 李⑸쪠?댁빞 ??寃?媛숈븘.' }
  ])
  ...
  const getAiDialogue = async () => {
    ...
    const prompt = `
      留뚰솕 而??쒖옉 ???諛??꾩씠?붿뼱 異붿쿇:
      - ?꾩옱 留뚰솕 而??ㅻ챸: "${activeCut.description || '????놁씠 而룸쭔 ?앹꽦??}"
      - ?묒꽦??湲곗〈 ??? "${activeCut.dialogue || '鍮꾩뼱 ?덉쓬'}"
      
      ???곹솴???뚮쭪? 罹먮┃?곗쓽 ?ъ튂 ?덇퀬 紐곗엯???믪? ????먮뒗 ?ㅼ쓬 而룹쑝濡??댁뼱媛??ㅽ넗由??꾩씠?붿뼱瑜?3媛吏 ?쒖븞??以?
      諛섎뱶??媛??쒖븞? 踰덊샇 ?놁씠 ??以꾩뵫 ?묒꽦??二쇨퀬, 媛곴컖???쒖븞? 以꾨컮轅?\n) 臾몄옄濡쒕쭔 援щ텇??以?
    `
    ...
    try {
      ...
      if (lines.length > 0) {
        ...
        setAiSuggestions([
          '?ш릿 ?덈Т ?대몢??.. ?꾨벑??李얠븘蹂댁옄!',
          '? 硫由?湲곗?泥섎읆 蹂댁씠??遺덈튆??源쒕묀?닿퀬 ?덉뼱.',
          '?댁찈硫??멸퀎 ?앸챸泥닿? ?곕┫ 蹂닿퀬 ?덉쓣吏??紐곕씪.'
        ])
      }
    } catch (err) {
      console.error('Failed to query Gemini:', err)
      setAiError('Gemini API ?몄텧???ㅽ뙣?섏뿬 湲곕낯 ?쒗뵆由???щ? 異붿쿇?⑸땲??')
      setAiSuggestions([
        '?곕즺媛 ?꾩쟾??硫덉톬?? ?곗냼 ?좎?瑜?理쒕?濡??щ젮!',
        '?대뼸寃뚮뱺 ?듭떊 ?μ튂瑜?蹂듦뎄?댁꽌 吏援?蹂몃????뚮━??',
        '???앷컖???됱꽦 吏?쒕㈃ ?꾨옒???먮꼫吏 愿묐Ъ??留ㅼ옣?섏뼱 ?덉쓣 寃?媛숈븘.'
      ])
    } finally {
      setAiLoading(false)
    }
  ...
  const handleSave = async (isDraft: boolean) => {
    ...
    try {
      ...
      setDbStatus({
        type: 'success',
        msg: isDraft ? '?꾩떆 ??μ씠 ?꾨즺?섏뿀?듬땲??' : '?깃났?곸쑝濡???λ릺?덉뒿?덈떎.'
      })
    } catch (err) {
      ...
      setDbStatus({
        type: 'local',
        msg: `?섑띁踰좎씠??toon_projects ?뚯씠釉??곌껐 ?ㅽ뙣濡?濡쒖뺄 ??μ냼(localStorage)???덉쟾?섍쾶 諛깆뾽?섏뿀?듬땲?? (${err instanceof Error ? err.message : '?ㅽ듃?뚰겕 ?곹깭 ?뺤씤 ?붾쭩'})`
      })
    } finally {
      setDbLoading(false)
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      setDbStatus({
        type: 'local',
        msg: `?섑띁踰좎씠??toon_projects ?뚯씠釉?諛고룷 ?ㅽ뙣濡?濡쒖뺄 ??μ냼(localStorage)???덉쟾?섍쾶 諛고룷?섏뿀?듬땲??`
      })
    } finally {
      setDbLoading(false)
      setTimeout(() => setDbStatus(null), 5000)
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-850 pb-5">
        <div className="space-y-1.5 w-full sm:max-w-md">
          ...
          />
          <p className="text-[10px] text-slate-500 pl-2.5">?곗뒪荑?AI ?쒕줈???먮뵒??罹붾쾭???댄궥</p>
        </div>
        ...
        <div className="lg:col-span-3 space-y-4">
          ...
          <div className="flex justify-between items-center p-3.5 bg-slate-900 border border-slate-850 rounded-2xl">
            <span className="text-xs text-slate-450 font-bold">
              珥?留뚰솕 而??? <span className="text-purple-400">{cuts.length}媛?/span>
            </span>
            ...
            {cuts.map((cut, index) => (
              ...
              >
                ...
                <div className="w-full md:w-56 h-36 rounded-2xl bg-slate-950 border border-slate-850 flex flex-col items-center justify-center shrink-0 text-center gap-2 relative overflow-hidden">
                  ...
                  <span className="text-[10px] text-slate-600 font-bold uppercase tracking-wider">AI ?대?吏 留ㅽ븨 怨듦컙</span>
                </div>

                {/* Edit fields */}
                <div className="flex-1 space-y-3 pt-2">
                  <div className="space-y-1">
                    <label className="text-[9px] font-bold text-slate-500">而??쒓컖 臾섏궗 (AI 洹몃┝???꾨＼?꾪듃)</label>
                    <textarea
                      value={cut.description}
                      onChange={(e) => updateCutField(cut.id, 'description', e.target.value)}
                      placeholder="?? ?곗＜蹂듭쓣 ?낆? 洹?ъ슫 ?멸퀎?몄씠 ?먯쓣 ?붾뱾硫??섑븯寃??껉퀬 ?덈떎."
                  ...
                  <div className="space-y-1">
                    <label className="text-[9px] font-bold text-slate-500">罹먮┃??留먰뭾?????(Dialogue)</label>
                    <input
                      type="text"
                      value={cut.dialogue}
                      onChange={(e) => updateCutField(cut.id, 'dialogue', e.target.value)}
                      placeholder="??而룹뿉 ?ㅼ뼱媛???щ? ?묒꽦??二쇱꽭??"
        ...
        <div className="space-y-4">
            ...
            <div className="w-full aspect-square rounded-2xl bg-gradient-to-br from-slate-950 to-slate-900 border border-slate-850 flex items-center justify-center overflow-hidden p-4">
              {selectedCharacterId && characters.length > 0 ? (
                ...
                <div className="text-center">
                  <p className="text-xs text-slate-500">罹먮┃?곕? ?좏깮??二쇱꽭??/p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="p-5 rounded-3xl bg-slate-900 border border-slate-800 space-y-5">
            <h3 className="font-bold text-xs text-slate-200 flex items-center gap-2 border-b border-slate-850 pb-2.5">
              ...
              <span>?쒕??섏씠 AI ???異붿쿇湲?/span>
            </h3>

            <div className="text-[10px] text-slate-500 bg-slate-950 border border-slate-850/60 p-3 rounded-xl">
              <p className="font-semibold text-slate-400">?좏깮??而? <span className="text-purple-400 font-bold">而?#{activeCutIndex + 1}</span></p>
              <p className="mt-1 font-medium leading-relaxed">?묒꽦?섏떊 而?臾섏궗? ???留λ씫??遺꾩꽍?섏뿬 ?댁슱由щ뒗 留먰뭾??硫섑듃? ?꾩씠?붿뼱瑜??앹꽦?⑸땲??</p>
            </div>
            ...
            >
              ...
              <span>{aiLoading ? 'AI ???遺꾩꽍 以?..' : 'AI ???異붿쿇諛쏄린'}</span>
            </button>
            ...
            {aiSuggestions.length > 0 && (
              <div className="space-y-2 pt-2 border-t border-slate-850">
                <h4 className="text-[9px] font-bold text-slate-500 uppercase tracking-wide">?대┃ ????ъ갹??諛붾줈 ?낅젰</h4>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 max-w-md w-full relative overflow-hidden shadow-2xl">
            ...
            <div className="flex flex-col items-center text-center space-y-4 pt-2">
              ...
              <div className="space-y-1.5">
                <h3 className="text-lg font-bold text-slate-100">留뚰솕 諛고룷 ?깃났!</h3>
                <p className="text-xs text-slate-450 leading-relaxed">
                  ?묒꽦?섏떊 ?뱁댆?????멸퀎??怨듦컻?섏뿀?듬땲??<br />
                  ?꾨옒 怨좎쑀 二쇱냼瑜??듯빐 濡쒓렇???놁씠 ?꾧뎄??媛먯긽?????덉뒿?덈떎.
                </p>
              </div>
